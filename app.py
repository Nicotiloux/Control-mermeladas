...
VENTAS_HEADERS = [
    ...
    "subtotal_bruto", "total", "estado_pago", "estado_entrega", "estado_venta", "es_historica",
]
...
# Costo por unidad que se usa cuando un sabor todavía no tiene "precio costo" cargado
# en Inventario (por ejemplo, ventas históricas de sabores que nunca se gestionaron ahí).
# Si cargas un precio_costo específico para un sabor en Inventario, ese valor manda.
COSTO_UNITARIO_DEFAULT = 2700
...
# Caché de lectura: vive solo durante UNA recarga de la página (se reinicia solo en
# cada rerun de Streamlit, porque este diccionario se vuelve a crear vacío cada vez
# que el script se ejecuta de nuevo). Evita leer la misma hoja varias veces en una
# sola pantalla, que era la causa principal de toparse con el límite de la API.
_lectura_cache = {}


def _con_reintento(func, intentos=4, espera_base=2):
    """
    Ejecuta una llamada a la API de Google Sheets reintentando con espera creciente
    si Google responde con un error temporal (por ejemplo, demasiadas solicitudes
    seguidas). Si tras varios intentos sigue fallando, deja pasar el error real.
    """
    import time
    ultimo_error = None
...
def get_or_create_ws(nombre, headers):
    ...
    sh = get_spreadsheet()
    try:
        ws = _con_reintento(lambda: sh.worksheet(nombre))
    except gspread.WorksheetNotFound:
        ws = sh.add_worksheet(title=nombre, rows=2000, cols=max(len(headers), 10))
        _con_reintento(lambda: ws.append_row(headers))
    cache[nombre] = ws
    return ws


def leer_hoja(nombre, headers, columnas_numericas=None, usar_cache=True):
    if usar_cache and nombre in _lectura_cache:
        df = _lectura_cache[nombre].copy()
    else:
        ws = get_or_create_ws(nombre, headers)
        registros = _con_reintento(lambda: ws.get_all_records())
...
def escribir_hoja(nombre, headers, df):
    ...
    _con_reintento(lambda: ws.clear())
    _con_reintento(lambda: ws.update(filas, value_input_option="USER_ENTERED"))
    _lectura_cache[nombre] = df.copy()  # mantener la caché al día tras escribir
...
def obtener_comprometido_por_sabor():
    """Unidades comprometidas en ventas ABIERTAS y no históricas (las históricas ya
    ocurrieron en el pasado y no representan algo pendiente de producir)."""
    ventas_abiertas_df = obtener_ventas("Abierta")
    if not ventas_abiertas_df.empty:
        ventas_abiertas_df = ventas_abiertas_df[ventas_abiertas_df["es_historica"] != "Sí"]
...
def obtener_ventas(estado=None):
    ...
    if not df.empty:
        df["es_historica"] = df["es_historica"].replace("", "No").fillna("No")
        df.loc[~df["es_historica"].isin(["Sí", "No"]), "es_historica"] = "No"
...
def registrar_venta_multiple(cliente, lugar, fecha_venta, fecha_entrega, descuento, carrito, es_historica=False):
    ...
    nueva_venta = {
        ...
        "es_historica": "Sí" if es_historica else "No",
    }
    ...
    if es_historica:
        return nuevo_id_venta, "Venta histórica registrada correctamente."
...
def actualizar_venta_completa(venta_id, cliente, lugar, fecha_venta, fecha_entrega, descuento, carrito, es_historica=False):
    ...
    ventas_df.at[idx, "es_historica"] = "Sí" if es_historica else "No"
...
def anular_venta_cerrada(venta_id):
    ...
    if ventas_df.at[idx, "es_historica"] != "Sí":
        inventario_df = obtener_inventario()
    ...
    escribir_hoja("ventas", VENTAS_HEADERS, ventas_df)
    if ventas_df.at[idx, "es_historica"] == "Sí":
        return True, f"Venta histórica #{venta_id} anulada (no se tocó el stock, esta venta nunca lo descontó)."
...
def actualizar_estado_venta(venta_id, nuevo_pago, nueva_entrega):
    ...
    items = obtener_detalle(venta_id)
    es_historica = ventas_df.at[idx, "es_historica"] == "Sí"

    if nuevo_pago == "Pagado" and nueva_entrega == "Entregado":
        if es_historica:
            # Venta histórica: se cierra directo, sin revisar ni descontar stock actual,
            # porque ese producto ya se vendió en el pasado y no representa stock de hoy.
            ventas_df.at[idx, "estado_venta"] = "Cerrada"
            escribir_hoja("ventas", VENTAS_HEADERS, ventas_df)
            return "cerrada", f"Venta histórica #{venta_id} cerrada (no se tocó el stock actual)."
...
def calcular_costo_total(detalle_df, costos_por_sabor):
    """Suma cantidad * costo unitario para cada fila de un detalle de ventas, usando
    COSTO_UNITARIO_DEFAULT cuando el sabor no tiene precio_costo cargado en Inventario."""
    if detalle_df.empty:
        return 0
    return sum(
        row["cantidad"] * (costos_por_sabor.get(row["sabor"], 0) or COSTO_UNITARIO_DEFAULT)
...
if pagina == "📊 Resumen":
    ...
    col4.metric("Ingresos cobrados (histórico, bruto)", clp(ingresos_totales))

    detalle_cerradas_historico = obtener_detalle_ventas_cerradas()
    costos_por_sabor = inventario_df.set_index("sabor")["precio_costo"].to_dict() if not inventario_df.empty else {}
    costo_total_historico = calcular_costo_total(detalle_cerradas_historico, costos_por_sabor)
    ingresos_liquidos_historico = ingresos_totales - costo_total_historico

    col5, col6 = st.columns(2)
    col5.metric("Costo histórico total", clp(costo_total_historico))
    col6.metric("Ingresos líquidos (histórico, ganancia)", clp(ingresos_liquidos_historico))
    ...
    if ventas_cerradas_df.empty:
        ...
        detalle_mes = detalle_cerradas_historico[detalle_cerradas_historico["venta_id"].isin(df_mes["id"])]
        ...
        costo_mes = calcular_costo_total(detalle_mes, costos_por_sabor)
        ...
        st.caption(
            f"El costo usa el 'precio costo' que tienes cargado hoy en Inventario para cada "
            f"sabor, y {clp(COSTO_UNITARIO_DEFAULT)} por defecto para los que todavía no tienen "
            f"uno cargado. Si tus costos cambiaron con el tiempo, los meses pasados se recalculan "
            f"con el costo actual."
        )
...
# =======================================================================================
    ...
    if inventario_df.empty:
        ...
        with st.form(f"form_confirmar_venta_{sufijo_key}", clear_on_submit=True):
            c1, c2 = st.columns(2)
            ...
            es_historica = st.checkbox(
                "📜 Venta antigua / histórica (solo para estadísticas: al cerrarla NO se revisa "
                "ni se descuenta el stock actual)",
                value=default_es_historica,
            )
                ...
                if not st.session_state.carrito:
                    ...
                    if editando:
                        ok, msg = actualizar_venta_completa(
                            editando, cliente, lugar, fecha_venta, fecha_entrega, descuento,
                            st.session_state.carrito, es_historica,
                        )
                        st.session_state.editando_venta_id = None
                    else:
                        ok, msg = True, registrar_venta_multiple(
                            cliente, lugar, fecha_venta, fecha_entrega, descuento,
                            st.session_state.carrito, es_historica,
                        )[1]
                    st.session_state.carrito = []

                    # Limpiar los widgets que viven FUERA del form (el form con
                    # clear_on_submit=True ya limpia lugar/descuento/fechas/histórica solo)
                    # para que el cliente y el selector de productos queden listos de cero.
                    claves_a_limpiar = [
                        k for k in list(st.session_state.keys())
                        if k.startswith("cliente_sel_") or k.startswith("cliente_nuevo_")
                        or k in ("sabor_carrito", "cant_carrito")
                        or k.startswith("precio_carrito_")
                    ]
    ...
    if ventas_abiertas_df.empty:
        ...
        with st.expander("Ver productos de esta venta", expanded=True):
            ...
            etiqueta_historica = " · 📜 Venta histórica" if fila_venta["es_historica"] == "Sí" else ""
            st.caption(
                f"Fecha venta: {fila_venta['fecha_venta']} · Fecha entrega: {fila_venta['fecha_entrega']} · "
                f"Descuento: {clp(fila_venta['descuento'])} · Total: {clp(fila_venta['total'])}{etiqueta_historica}"
            )
