...
# =======================================================================================
    ...
    inventario_df = obtener_inventario()
    sabores_ya_en_inventario = set(inventario_df["sabor"].tolist()) if not inventario_df.empty else set()
    sabores_para_agregar = [s for s in SABORES_DISPONIBLES if s not in sabores_ya_en_inventario]

    with st.expander("➕ Agregar nuevo sabor", expanded=False):
        if not sabores_para_agregar:
            st.info("Ya agregaste todos los sabores de la lista predefinida.")
        else:
            with st.form("form_agregar_sabor", clear_on_submit=True):
                c1, c2, c3, c4 = st.columns(4)
                nuevo_sabor = c1.selectbox("Sabor / variedad", sabores_para_agregar)
    ...
    if inventario_df.empty:
        ...
        if not faltan.empty:
            resumen_falta = ", ".join(f"{r.Sabor}: {r['Falta producir']}" for r in faltan.itertuples())
...
# =======================================================================================
    ...
    if inventario_df.empty:
        ...
        if c3.button("➕ Agregar al carrito", use_container_width=True):
            ...
            if fila_inv.empty:
                st.warning(f"'{sabor_sel}' todavía no está en tu inventario. Agrégalo primero en 📦 Inventario.")
            else:
                precio = float(fila_inv["precio_venta"].values[0])
